...
twtgui::TwtDownloader::TwtDownloader()
{
    std::error_code ec;
    if (!std::filesystem::exists("cacert.pem", ec))
    {
...
twtgui::TwtDownloader::~TwtDownloader() = default;

void twtgui::TwtDownloader::ensureCurlInit()
{
    std::call_once(s_curlInitFlag, []()
                   { curl_global_init(CURL_GLOBAL_DEFAULT); });
}

// resolve CA bundle path from explicit arg or environment fallbacks
static std::string resolveCaBundle(const std::string &explicitPath)
{
    if (!explicitPath.empty())
        return explicitPath;
    const char *env1 = std::getenv("TWTGUI_CURL_CA_BUNDLE");
    if (env1 && env1[0])
        return std::string(env1);
    const char *env2 = std::getenv("CURL_CA_BUNDLE");
    if (env2 && env2[0])
        return std::string(env2);
    return std::string();
}
static size_t writeToFileCallback(void *ptr, size_t size, size_t nmemb, void *userdata)
{
...
static size_t writeToStringCallback(void *ptr, size_t size, size_t nmemb, void *userdata)
{
...
twtgui::TwtDownloader::Result twtgui::TwtDownloader::downloadToFile(const std::string &url, const std::string &outputPath, long timeoutSeconds, bool verifyPeer)
{
    Result res;
    ...
    if (!curl)
    {
        res.error = "twtGUI: curl_easy_init() failed";
        return res;
    }
    ...
    if (!file)
    {
        res.error = "twtGUI: Failed to open output file: " + outputPath;
    ...
    curl_easy_setopt(curl, CURLOPT_FAILONERROR, 1L); // causes HTTP errors >=400 to fail
    ...
    std::string resolvedCa = resolveCaBundle("cacert.pem");
    if (!resolvedCa.empty())
    {
        curl_easy_setopt(curl, CURLOPT_CAINFO, resolvedCa.c_str());
    }
    ...
    if (code != CURLE_OK)
    {
        ...
        // try to get HTTP code anyway
        curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &res.http_code);
        // for SSL errors, include numeric code for diagnostics
        res.error += std::string(" (curl code: ") + std::to_string(static_cast<int>(code)) + ")";
    }
    else
    {
        curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &res.http_code);
        long pos = ftell(file);
        if (pos >= 0)
            res.bytesDownloaded = static_cast<std::size_t>(pos);
        res.success = true;
    }
...
twtgui::TwtDownloader::Result twtgui::TwtDownloader::downloadToString(const std::string &url, std::string &outData, long timeoutSeconds, bool verifyPeer)
{
    Result res;
    ...
    {
        curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &res.http_code);
        res.bytesDownloaded = outData.size();
        res.success = true;
    }
